...
// INSERT POINT: _wheel
	...
	reorderInfinite: function () {
		...
		if ( this.cachePhase != cachePhase && (cachePhase === 0 || minorPhase - this.infiniteCacheBuffer > 0) ) {
			//this.options.dataset.call(this, Math.max(cachePhase * this.infiniteCacheBuffer - this.infiniteCacheBuffer, 0), this.options.cacheSize); // DEL BY AL
			this.options.dataset.call(this, Math.max(cachePhase * this.options.cacheSize - this.options.cacheSize, 0), this.options.cacheSize); // ADD BY AL
		}
	...
	updateCache: function (start, data) {
		// var firstRun = this.infiniteCache === undefined; // DEL BY AL

		//this.infiniteCache = {}; // DEL BY AL

		for ( var i = 0, l = data.length; i < l; i++ ) {
			this.infiniteCache[start++] = data[i];
		}

		//if ( firstRun ) { // DEL BY AL
